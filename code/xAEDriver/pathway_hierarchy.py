...
class ReactomeNetwork():
    ...
    def __init__(self, pathway_names, pathway_genes, relations_file_name, species):
        self.reactome = Reactome(pathway_names, pathway_genes, relations_file_name, species)  
...
def get_masking(pathway_names, pathway_genes, relations_file_name, data, n_fakeSNP, n_dep, n_expr, species='human', n_hidden=3):
    ...
    genes_df = gene_mapping(data.index.tolist(), genes_df)
    ...
    for i in range(n_level):
        ...
        layers_node[i] = nodes
    layers_node[0] = list(set(range(0, n_fakeSNP)))
    original_gene = list(data.index)
    gene_dep = list(set(original_gene[0: n_dep]) & set(layers_node[n_level - 1]))
    gene_expr = list(set(original_gene[n_dep: n_dep + n_expr]) & set(layers_node[n_level - 1]))
    layers_node[n_level - 1] = gene_dep + gene_expr
    ...
    # Update the dependency score data and expression data for the filtered genes
    data_dep = data.iloc[0:n_dep, :]
    data_expr = data.iloc[n_dep:n_dep+n_expr, :]
    data_dep = data_dep.loc[gene_dep, :]
    data_expr = data_expr.loc[gene_expr, :]
    data_dep.index = [str(index) + "-dependency" for index in data_dep.index]
    data_expr.index = [str(index) + "-expression" for index in data_expr.index]
    data = pd.concat([data_dep, data_expr], axis=0)
    ...
    return masking, layers_node, data
